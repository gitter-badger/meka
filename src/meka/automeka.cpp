...

      {
        ...
        for (auto const& p : projects) {
          for (auto const& o : p.binaries) {
            out << "build " << (bindir / p.name / fs::basename(o)).string() << ": exe " << o.string() << "\n";
            out << "  libs = -lboost_system -lboost_filesystem -lboost_program_options -lboost_regex " << boost::algorithm::join(archives, " ") << "\n";
            out << "\n";
          }
        }
      }
